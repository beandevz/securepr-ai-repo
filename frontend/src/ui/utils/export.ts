...
/**
 * Export findings summary for JIRA/Linear
 */
export function exportForJira(result: ScanResult): string {
  const jira = `h1. Security Scan Report - PR #${result.pr_number}

*Repository:* ${result.owner}/${result.repo}
*Pull Request:* #${result.pr_number} - ${result.pr_title}
*Author:* ${result.author}
*Verdict:* ${result.verdict === 'PASS' ? '(/)' : '(x)'} ${result.verdict}
*Score:* ${result.score}/${result.threshold}

h2. Findings (${result.findings.length})
...
/**
 * Copy JIRA-formatted text to clipboard
 */
export async function copyForJira(result: ScanResult) {
  const jira = exportForJira(result);
  try {
